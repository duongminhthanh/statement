import { UserDemo2ModalComponent } from './user-demo2-modal/user-demo2-modal.component';
import {Component, ElementRef, OnInit, ViewChild} from '@angular/core';
import {NgbModal} from '@ng-bootstrap/ng-bootstrap';
import {Subject} from 'rxjs';
import {takeUntil} from 'rxjs/operators';
import { User2Service } from 'src/app/core/services/module/user2.service';
...
@Component({
  ...
  templateUrl: './user-demo2.component.html',
  styleUrls: ['./user-demo2.component.scss']
})
export class UserDemo2Component implements OnInit {
  ...
  constructor(
      private user2Serivice: User2Service,
  ...
  ngOnInit() {
      this.breadCrumbItems = [{label: 'Home page', path: '/'},
          ...
          {label: 'User Demo2', path: '/', active: true}];
  ...
  getUserList() {
      ...
      this.loading = true;
      this.user2Serivice.getUserList(json).subscribe(res => {
          if (res) {
  ...
  deletedUser(item: any) {
      Swal.fire({
          ...
          if (result.value) {
              ...
              this.loading = true;
              this.user2Serivice.deleteUserDemo(json).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
  ...
  openCreateModal(item: any) {
      const modalRef = this.modalService.open(UserDemo2ModalComponent, {centered: true, size: 'lg'});
      if (item) {
  ...
  onFileSelect(e: any) {
      if (e.target.files ? e.target.files.length > 0 : false) {
          this.user2Serivice.importUsers(
